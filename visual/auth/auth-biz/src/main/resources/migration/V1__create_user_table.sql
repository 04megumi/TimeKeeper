...
CREATE TABLE IF NOT EXISTS user (
    uid         VARCHAR(64)   PRIMARY KEY NOT NULL   COMMENT '用户唯一标识（UID）',
    ...
    openid      VARCHAR(255)                         COMMENT '第三方登录唯一标识（OpenID）',
    role        VARCHAR(255)                         COMMENT '用户角色(学生/教师/管理员) ',
...
CREATE UNIQUE INDEX idx_user_openid    ON user(openid, name);